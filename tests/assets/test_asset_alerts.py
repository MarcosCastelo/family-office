...
def test_alerta_concentracao_gerado(client, headers, family, asset_payload):
    ...
    import uuid
    asset1 = asset_payload.copy(); asset1["value"] = 70000.0; asset1["name"] = "Ativo Conc " + str(uuid.uuid4())
    asset2 = asset_payload.copy(); asset2["value"] = 20000.0; asset2["name"] = "Ativo Conc " + str(uuid.uuid4())
...
def test_alerta_nao_duplicado(client, headers, family, asset_payload):
    ...
    import uuid
    asset1 = asset_payload.copy(); asset1["value"] = 90000.0; asset1["name"] = "Alerta Não Duplicado 1 " + str(uuid.uuid4())
    asset2 = asset_payload.copy(); asset2["value"] = 10000.0; asset2["name"] = "Alerta Não Duplicado 2 " + str(uuid.uuid4())
    resp1 = client.post("/assets", json=asset1, headers=headers)
    resp2 = client.post("/assets", json=asset2, headers=headers)
    assert resp1.status_code == 201
    assert resp2.status_code == 201
...
def test_alerta_liquidez_gerado(client, headers, family):
    ...
    client.post('/assets/upload', content_type='multipart/form-data', headers=headers, data=data)
